...
var Vim = function Vim() {
	...
	function cmKey(key, cm) {
		...
		if (typeof cmd == 'function') {
			_cm_adapter["default"].signal(cm, 'vim-keypress', vimKey); // 广播按下事件
		}
	...
	function maybeInitVimState(cm) {
		if (!cm.state.vim) {
			// Store instance state in the CodeMirror object. 在CodeMirror对象中存储实例状态。
			cm.state.vim = {
				inputState: new InputState(),
				// Vim's input state that triggered the last edit, used to repeat Vim的输入状态，它触发最后一次编辑，用于重复
				// motions and operators with '.'. 带有“。”的动作和操作符。
				lastEditInputState: undefined,
				// Vim's action command before the last edit, used to repeat actions Vim在最后一次编辑之前的操作命令，用于重复操作
	...
	function resetVimGlobalState() {
		vimGlobalState = {
			// The current search query. 当前搜索查询。
			searchQuery: null,
			// Whether we are searching backwards. 我们是否在向后搜索。
			searchIsReversed: false,
			// Replace part of the last substituted pattern 替换最后一个替换模式的一部分
			lastSubstituteReplacePart: undefined,
			jumpList: createCircularJumpList(),
			macroModeState: new MacroModeState(),
			// Recording latest f, t, F or T motion command. 记录最新的f, t, f或t运动命令。
			lastCharacterSearch: {
				increment: 0,
				forward: true,
				selectedCharacter: ''
	...
	var vimApi = {
		...
		findKey: function findKey(cm, key, origin) {
			...
			function handleEsc() {
				if (key == '<Esc>') {
					// Clear input state and get back to normal mode. 清除输入状态，回到正常模式。
					let range = cm.getSelectionRange();
					clearInputState(cm);
					...
					if (vim.visualMode) {
						...
						if(range && !range.isEmpty()){
							cm.setVisualSelection(range);
						}
					}else{
						// 清除多选的光标
						cm.editor.setSelection(cm.editor.getSelection())
					}
	...
	var commandDispatcher = {
		...
		processMotion: function processMotion(cm, vim, command) {
			vim.inputState.motion = command.motion;// 运动类型
			vim.inputState.motionArgs = copyArgs(command.motionArgs); // 运动参数 反向之类 单词之类
		...
		evalInput: function evalInput(cm, vim) {
			// If the motion command is set, execute both the operator and motion. 如果设置了motion命令，则同时执行操作符和motion。
			// Otherwise return.
			var inputState = vim.inputState;
			...
			var sel = vim.sel; // TODO: Make sure cm and vim selections are identical outside visual mode. 待办事项:确保cm和vim的选择是相同的外部视觉模式。

			var origHead = copyCursor(vim.visualMode ? clipCursorToContent(cm, sel.head) : cm.getCursor('head'));
			var origAnchor = copyCursor(vim.visualMode ? clipCursorToContent(cm, sel.anchor) : cm.getCursor('anchor'));
			// var origHead = copyCursor(cm.getCursor('head'));
			// var origAnchor = copyCursor(cm.getCursor('anchor'));
			...
			if (inputState.repeatOverride !== undefined) {
				// If repeatOverride is specified, that takes precedence over the 如果指定了repeatOverride，则该参数优先于
				// input state's repeat. Used by Ex mode and can be user defined. 输入的重复。使用Ex模式，并可由用户定义
			...
			// 选择字符行动
			if (inputState.selectedCharacter) {
				// If there is a character input, stick it in all of the arg arrays. 如果有字符输入，请将其插入所有的arg数组中。
			...
			if (motion) {
				...
				if (motionArgs.toJumplist) {
					var jumpList = vimGlobalState.jumpList; // if the current motion is # or *, use cachedCursor 如果当前运动是#或*，使用缓存的游标
				...
				} // TODO: Handle null returns from motion commands better. 待办事项:更好地处理动作命令的null返回。
			...
			if (operator) {
				if (operatorArgs.lastSel) {
					// Replaying a visual mode operation 重放视觉模式操作
					newAnchor = oldAnchor;
					...
					if (lastSel.visualLine) {
						// Linewise Visual mode: The same number of lines. 行视方式:相同的行数。
						newHead = Pos(oldAnchor.line + lineOffset, oldAnchor.ch);
					} else if (lastSel.visualBlock) {
						// Blockwise Visual mode: The same number of lines and columns. 分段视觉模式:相同的行数和列数。
						newHead = Pos(oldAnchor.line + lineOffset, oldAnchor.ch + chOffset);
					} else if (lastSel.head.line == lastSel.anchor.line) {
						// Normal Visual mode within one line: The same number of characters. 单行正常视觉模式:字符数相同。
						newHead = Pos(oldAnchor.line, oldAnchor.ch + chOffset);
					} else {
						// Normal Visual mode with several lines: The same number of lines, in the 普通视觉模式多行:相同的行数，在
						// last line the same number of characters as in the last line the last time. 最后一行与上次最后一行相同的字符数。
						newHead = Pos(oldAnchor.line + lineOffset, oldAnchor.ch);
					}
				...
				if (vim.visualMode) {
					...
					if (linewise) {
						...
						if (mode == 'block') {
							// Linewise operators in visual block mode extend to end of line 视块模式下的行操作符延伸到行尾
							for (var i = 0; i < ranges.length; i++) {
					...
					if (linewise) {
						// Expand selection to entire line. 将选择展开到整行。
						expandSelectionToLine(cm, curStart, curEnd);
					} else if (motionArgs.forward) {
						// Clip to trailing newlines only if the motion goes forward. 只有当动作继续时才剪辑到后面的换行符。
				...
				vim.lastMotion = null;
				operatorArgs.repeat = repeat; // For indent in visual mode. 用于在可视模式下缩进。

				operatorArgs.registerName = registerName; // Keep track of linewise as it affects how paste and change behave. 当它影响粘贴和更改的行为时，按行跟踪。
	...
	* match the input selection.
	...
	var actions = {
		...
		updateFakeCursor:updateFakeCursor,//修改
		...
		enterInsertMode: function enterInsertMode(cm, actionArgs, vim) {
			...
			var height = 1//cm.listSelections().length;
		...
		toggleVisualMode: function toggleVisualMode(cm, actionArgs, vim) {
			...
			if (!vim.visualMode) {
				...
				exitVisualMode(cm,actionArgs.moveHead);
			}
		},
	 ...
	 * 更新CodeMirror选择，以匹配提供的vim选择。
	 * 如果没有给出参数，则使用当前的vim选择状态。
	 */
	...
	function updateCmSelection(cm, sel, mode) {
		...
		var ranges = cmSel.ranges;
		// 兼容 vscode 常规模式的选择
		if(ranges[0].anchor.ch<=ranges[0].head.ch && ranges[0].anchor.line<=ranges[0].head.line)
		{
			ranges =  [{
				anchor: ranges[0].anchor,
				head: {
					ch:ranges[0].head.ch+1,
					line:ranges[0].head.line,
				}
			}];
		}
		// var cmSel = makeCmSelection(cm, sel, mode, false);// vim.visualMode && mode === 'char');
		// 设置光标选择位置
		cm.setSelections(ranges, cmSel.primary);
		// 更新假光标
		updateFakeCursor(cm);
	}
