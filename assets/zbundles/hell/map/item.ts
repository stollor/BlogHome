...
export class TestItem extends Component {
	...
	start() {
		...
		this.baseMove.nowVelocity.set(Math.sin((this.baseMove.dir * 2 * Math.PI) / 360), Math.cos((this.baseMove.dir * 2 * Math.PI) / 360), 0);
		this.baseMove.nowVelocity.multiplyScalar(this.baseMove.speed);
		this.bounds = new Rect(this.node.position.x - this._ui.anchorX * this._ui.width, this.node.position.y - this._ui.anchorY * this._ui.height, this._ui.width, this._ui.height);
		// 注册单个碰撞体的回调函数
	...
	checkCollision(quadTree: QuadTree) {
		this.bounds = new Rect(this.node.position.x - this.ui.anchorX * this.ui.width, this.node.position.y - this.ui.anchorY * this.ui.height, this.ui.width, this.ui.height);
		quadTree.retrieveNode(this.node).forEach((item) => {
			if (item.node.uuid != this.node.uuid) {
				if (!(this.bounds.x > item.x || this.bounds.x + this.bounds.width < item.x || this.bounds.y > item.y || this.bounds.y + this.bounds.height < item.y)) {
