import { Color, Component, EventMouse, Graphics, Input, Node, Rect, Sprite, UITransform, _decorator, input, instantiate, v3 } from 'cc';
import { BaseMove } from '../../quadTree/BaseMove';
...
export class quadTest extends Component {
	...
	refreshQuadTree() {
		this._quadTree = new QuadTree(new Rect(this.node.position.x - this._ui.anchorX * this._ui.width, this.node.position.y - this._ui.anchorY * this._ui.height, this._ui.width, this._ui.height), 10, 3);
	...
	createItem(num: number) {
		for (let i = 0; i < num; i++) {
			...
			item.active = true;
			item.setPosition(this.node.position.x - this._ui.anchorX * this._ui.width + Math.random() * this._ui.width, this.node.position.y - this._ui.anchorY * this._ui.height + Math.random() * this._ui.height);
			item.getComponent(TestItem).edges = new Rect(this.node.position.x - this._ui.anchorX * this._ui.width, this.node.position.y - this._ui.anchorY * this._ui.height, this._ui.width, this._ui.height);
