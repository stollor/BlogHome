import { EventMouse, EventTouch, Graphics, Node, UITransform, Vec3, Widget, _decorator, easing, macro } from 'cc';
import { Manager } from '../abstract/Manager';
import { ScrollAdapter } from '../abstract/ScrollAdapter';
import { ADAPTER_DEBUG_CONTENT, DEBUG_DRAW_BORDER_COLOR, DEBUG_DRAW_FILL_COLOR, DEBUG_DRAW_LIND_WIDTH } from '../define/debug';
...
export class ScrollManager extends Manager {
	...
	@property(UITransform) get content() {
	...
	@property({ type: Orientation }) get orientation() {
	...
	public get laseScrollDirection() {
	...
	public get boundaryOffset() {
		return this._boundaryOffset;
	}
	public get parentAdapter() {
	...
	private _isMyEvent(event: EventTouch, useCaptures: Node[]) {
		if (
			event.eventPhase == EventTouch.AT_TARGET ||
			!useCaptures ||
			(useCaptures[0] == this.view.node && !event.target[ADAPTER])
		) {
			return true;
		}
		return false;
	...
	private _getContentMinBoundaryOffset(delta: number, position: number) {
		if (
			!this.adapter.viewManager.header ||
			(!this.adapter.centerManager.enabled && this.adapter.viewManager.virtualSize <= this.adapter.mainAxisSize)
		) {
			if (this.adapter.isHorizontal) {
	...
	public scrollToModelIndex(duration: number, index: number, alwaysScroll?: AlwaysScroll) {
		...
		var ok = this._scrollToPosition(
			duration,
			() => {
				var position = this.adapter.centerManager.getPositionByGroupIndex(
					groupIndex,
					alwaysScroll,
					priorityCheckExists
				);
