import { Node, UITransform, _decorator } from 'cc';
import { Manager } from '../abstract/Manager';
import { Layer, WrapMode } from '../define/enum';
import { IModel, InterData } from '../define/interface';
...
export class ModelManager<T = any> extends Manager {
	...
	public async insert(data: T | T[], interdataCB: (T: any) => InterData, insertIndex: number = this.length) {
		var list = this._toArray(data);
		insertIndex = Helper.clamp(insertIndex, 0, this.length);
		this.emit(Event.ON_CHANGE_BEFORE);
		this._insertHandler(list, interdataCB, insertIndex);
	...
	private _insertHandler(list: any[], interdataCB: (T: any) => InterData, insertIndex: number) {
		...
		const action = (data: T) => {
			...
			if (model) {
				...
				model.data = data;
				model.index = index;
				model.interdata = interdataCB(data);
				this._initModel(model);
			} else {
				let curr = newModel(data, index);
				curr.interdata = interdataCB(data);
	...
	private _initModel(model: IModel<T>): void {
		var prefab = this.adapter.getPrefab(model.interdata);
		if (!prefab) {
			throw Error('预制体不能为空');
		}
		if (prefab instanceof Node) {
			let transform = prefab.getComponent(UITransform);
			model.code = prefab.uuid;
			model.size = { x: transform.width, y: transform.height };
			model.scale = { x: transform.node.scale.x, y: transform.node.scale.y };
			model.anchorPoint = { x: transform.anchorPoint.x, y: transform.anchorPoint.y };
		} else {
			let transform = prefab.data.getComponent(UITransform);
		...
		model.element = {
			wrapBeforeMode: model.interdata.element.wrapBeforeMode ?? WrapMode.Wrap,
			wrapAfterMode: model.interdata.element.wrapAfterMode ?? WrapMode.Nowrap,
			ignoreLayout: model.interdata.element.ignoreLayout ?? false,
			placeholder: model.interdata.element.placeholder ?? false,
			minSize: model.interdata.element.minSize ?? { x: 0, y: 0 },
			preferredSize: model.interdata.element.preferredSize ?? { x: 0, y: 0 },
			flexibleSize: model.interdata.element.flexibleSize ?? { x: 0, y: 0 },
			layer: model.interdata.element.layer ?? Layer.Lowest,
			fixed: model.interdata.element.fixed ?? false,
			fixedOffset: model.interdata.element.fixedOffset ?? 0,
			fixedSpacing: model.interdata.element.fixedSpacing ?? null,
			update: () => {},
		};
