...
export class ViewManager<T = any> extends Manager {
	...
	public get groupLength() {
		return this._groupLength;
	}

	public get visibleLength() {
	...
	private _getMainAxisHeaderPosition(group: Group<T>) {
		...
		if (this.adapter.isVertical) {
			...
			anchor = this.adapter.isArrangeAxisStart
				? 1 - group.anchorPoint[this.adapter.mainAxis]
				: group.anchorPoint[this.adapter.mainAxis];
		} else {
	...
	public internal_getGroupHeaderBoundary(group: Group<T>) {
		...
		anchor = this.adapter.multiplier == 1 ? 1 - anchor : anchor;
		var value =
			group.position[this.adapter.mainAxis] +
	...
	public getMagneticOffset() {
		...
		if (this.magneticDirection == MagneticDirection.Footer) {
			...
			var value =
				Math.max(0, this.adapter.mainAxisSize - this.adapter.mainAxisPadding - this.getVisibleMainAxisSize()) *
				direction;
			return value;
		}
		return 0;
