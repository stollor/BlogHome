...
import { IElement, InterData } from '../define/interface';
...
export abstract class ScrollAdapter<T = any> extends Component {
	 ...
	 */
	@property({ type: ScrollManager, group: '滚动' }) scrollManager: ScrollManager = new ScrollManager();
	/**
	 * 视图管理器
	 * 负责管理可视区域item的创建、回收、主轴方向布局等逻辑
	 */
	@property({ type: ViewManager, group: '视图' }) viewManager: ViewManager<T> = new ViewManager();
	/**
	 * 布局管理器
	 * 负责交叉轴方向的布局逻辑
	 */
	@property({ type: LayoutManager, group: '布局' }) layoutManager: LayoutManager = new LayoutManager();
	/**
	 * 分页管理器
	 * 负责处理分页逻辑
	 */
	@property({ type: PageViewManager, group: '分页' }) pageViewManager: PageViewManager = new PageViewManager();
	/**
	 * 释放管理器
	 * 负责处理4个方向的 ”拉“ ”释放“ 等逻辑
	 */
	@property({ type: ReleaseManager, group: '释放' }) releaseManager: ReleaseManager = new ReleaseManager();
	/**
	 * 居中管理器
	 * 除了负责自动居中以外，还负责通过索引计算滚动的具体位置
	 */
	@property({ type: CenterManager, group: '居中' }) centerManager: CenterManager = new CenterManager();
	/**
	 * 数据管理器
	 ...
	 * @return 返回一个预制体
	 */
	public getPrefab(data: InterData): Node | Prefab {
		return this.prefabMap.get(data.prefab);
	}
	/**
 ...
 */
class DefaultHolder extends Holder {
	private _item!: VirtualItem;
	protected onCreated(): void {
		this._item = this.node.getComponent(VirtualItem) || this.node.addComponent(VirtualItem);
		this.node.emit(HolderEvent.CREATED, this);
		this._item.onCreat(this);
	}
	protected onVisible(): void {
