...
import { IElement, IModel, InterData } from '../define/interface';
...
export abstract class Holder<T = any, A extends ScrollAdapter<T> = ScrollAdapter<T>> {
	...
	public get interdata(): InterData | null {
		return this._model && this._model.interdata;
	}

	private _fixedIndex: number = -1;
	public get fixedIndex() {
	...
	constructor(node: Node, code: string, adapter: A) {
		this._node = node;
		this._code = code;
	...
	private _onSizeChanged() {
		...
		var isMainAxisEqual = Helper.approximately(
			this.model.size[this.adapter.mainAxis],
			this.transform.contentSize[this.adapter.mainAxis]
		);
		var isCrossAxisEqual = Helper.approximately(
			this.model.size[this.adapter.crossAxis],
			this.transform.contentSize[this.adapter.crossAxis]
		);
	...
	private _onScaleChanged(type: any) {
		...
		var isMainAxisEqual = Helper.approximately(
			this.model.scale[this.adapter.mainAxis],
			this.node.scale[this.adapter.mainAxis]
		);
		var isCrossAxisEqual = Helper.approximately(
			this.model.scale[this.adapter.crossAxis],
			this.node.scale[this.adapter.crossAxis]
		);
	...
	private _onAnchorPointChanged() {
		var isMainAxisEqual = Helper.approximately(
			this.model.anchorPoint[this.adapter.mainAxis],
			this.transform.anchorPoint[this.adapter.mainAxis]
		);
		var isCrossAxisEqual = Helper.approximately(
			this.model.anchorPoint[this.adapter.crossAxis],
			this.transform.anchorPoint[this.adapter.crossAxis]
		);
