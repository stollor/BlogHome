import { Director, System, cclegacy, director, gfx, js, macro } from 'cc';
...
export class DynamicAtlasManager extends System {
	...
	set enabled(value) {
		...
		if (value) {
			...
			this.reset();
			director.off(Director.EVENT_BEFORE_SCENE_LAUNCH, this.beforeSceneLoad, this);
		}
	...
	public insertSpriteFrame(spriteFrame) {
		...
		if (
			sampler.minFilter !== gfx.Filter.LINEAR ||
			sampler.magFilter !== gfx.Filter.LINEAR ||
			sampler.mipFilter !== gfx.Filter.NONE
		) {
			return null;
		}
